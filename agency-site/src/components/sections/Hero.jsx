...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 text-center pt-32 pb-24 md:pt-40 md:pb-32">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          <span className="inline-flex items-center gap-2 px-4 py-1.5 bg-indigo-500/10 border border-indigo-500/20 rounded-full text-indigo-400 text-xs font-semibold uppercase mb-8" style={{ letterSpacing: '0.05em' }}>
        ...
        <motion.div
          ...
          className="mt-20 md:mt-24 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 max-w-3xl mx-auto pt-12"
        ...
        >
          {[
            ...
            >
              <div className="text-2xl md:text-3xl font-bold group-hover:text-indigo-400 transition-colors duration-300" style={{ color: 'var(--text-primary)', fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '-0.03em' }}>{stat.value}</div>
              <div className="text-xs font-medium uppercase mt-2" style={{ color: 'var(--text-muted)', letterSpacing: '0.04em' }}>{stat.label}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
